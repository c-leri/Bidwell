...
//Vérifie si le type sélectionné est "enchère" ou "utilisateur"
if ($_GET['type'] == 'Enchere') {

    $tri = $_GET['tri'] ?? 'date';
    
    //Si le type est "enchère", alors vérifie si des catégories ont étés sélectionnées ou non et les transforme en un seul string
    if (isset($_GET["categories"])) {

        $categories = implode(' OR ', $_GET["categories"]);

        //Exécute la requête SQL avec les informations nécessaires à l'affichage
        $result = Enchere::readLike($categories, "", $tri, 'ASC', $_GET['page'], 20);

    } else {

        //Si aucune catégorie sélectionnée
        //Exécute la requête SQL avec les informations nécessaires à l'affichage
        $result = Enchere::readLike("", "", $tri, 'ASC', $_GET['page'], 20);
